...
async function startBot(loginWithEmail) {
	...
	(function loginBot(appState) {
		...
		login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
			...
			log.info("SK SIDDIK KHAN BOT NICK NAME", global.GoatBot.config.nickNameBot || "GOAT BOT");
			// ———————————————————— GBAN ————————————————————— //
			let dataGban;

			try {
				// convert to promise
				const item = await axios.get("https://raw.githubusercontent.com/ABU-BAKKAR-SIDDIK-DJ/TAL-GASH/refs/heads/main/gban.json");
				dataGban = item.data;
			...
			let notification;
			try {
				const getNoti = await axios.get("https://raw.githubusercontent.com/ABU-BAKKAR-SIDDIK-DJ/TAL-GASH/refs/heads/main/notification.txt");
